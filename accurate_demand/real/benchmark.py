...
class BenchmarkAccurateDemand:
    ...
    def Ampc_hat(self, d,p,TS,TD,x0):
        ...
        for t in range(T):
            try:
              pred_price = self.pred_step(t,t+2*W,"Price")
    ...
    def mpc(self):
        ...
        for n in range(1, n_interval+1):
            ...
            x,d,va,vb,cost = self.Ampc_hat(d_real,p_real,0,n*self.I,0)
            cost_mpc[n-1] = sum(cost)
        return cost_mpc
    ...
    def thb(self):
        ...
        for n in range(1, n_interval+1):
            ...
            cost_thb[n-1] = sum(sum(cost_shot))
        return cost_thb
    ...
    def deta(self):
        ...
        for n in range(1, n_interval+1):
            ...
            cost_deta[n-1] = sum(sum(cost_shot))
        return cost_deta
      
    def pred_step(self,t,T,MODEL_FLAG):
        ...
        VALID_SAT = len(data[self.N+self.V+t:,0])
        VALID_END = len(data[self.N+self.V+T:,0])
    ...
    def Aour_pred_transfer(self,abar,ts,tnz,T):
        #data = scaler.inverse_transform(scaled_data)
        test_price = self.P[self.N+self.V-1:self.N+self.V+T]
        ...
        for t in range(len(test_price)):
            ...
            try:
                prediction = self.pred_step(t,T,"Price")
                print(prediction)
            except:
    ...
    def leta(self):
        ...
        for n in range(1, n_interval+1):
            ...
            d,vb,cost[n-1] = self.Aour_pred_transfer(abar,ts,tnz,(n)*self.I)
        return cost
