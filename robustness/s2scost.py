# -*- coding: utf-8 -*-
"""
Created on Tue Jan 28 09:21:14 2020
...
if __name__ == "__main__":
    ...
    for n_month in [1,3,6,12]:
        for k in range(36):
            print(n_month,k)
            try:
                model_pv = Seq2Seq(output_dim=1, hidden_dim=10, output_length=n_outputs, input_shape=(BACK_DAY, dataset_r.shape[1]), peek=False, depth=2,dropout=0.2)
                model_price = Seq2Seq(output_dim=1, hidden_dim=10, output_length=n_outputs, input_shape=(BACK_DAY, dataset_p.shape[1]), peek=False, depth=2,dropout=0.2)
                model_pv.load_weights("./winddata/s2sr"+str(k)+str(n_month)+".h5")
                model_price.load_weights("./pricedata/s2sp"+str(k)+str(n_month)+".h5")
    ...
    np.save("./figure/s2slstm.npy",roblstm)
    np.save("./figure/onl.npy",robonl)
